...
% --- Setup Noise ---
Fs = 20000;             % Sampling frequency in Hz (Nyquist = 10kHz)
...
trans_width_lp = 500; % Transition band width in Hz
...
% --- Part (d): Band-Stop Filter ---
% (This code assumes 'noise', 'Fs', 'Ts' are still in the workspace)

fl_bs = 2000; % Lower cutoff of the stop-band in Hz
fh_bs = 5000; % Higher cutoff of the stop-band in Hz
trans_width_bs = 500; % Transition band width in Hz
nyquist_freq = Fs / 2;
filter_order = 100;

% Define frequency bands (normalized by Nyquist frequency)
% These are the same edges as the band-pass filter
f_bs_norm = [0, fl_bs-trans_width_bs/2, fl_bs+trans_width_bs/2, ...
             fh_bs-trans_width_bs/2, fh_bs+trans_width_bs/2, nyquist_freq] / nyquist_freq;

% Define desired amplitudes in those bands
% [Pass, Pass, Stop, Stop, Pass, Pass]
a_bs = [1, 1, 0, 0, 1, 1];

% Design the filter coefficients
b_bs = firpm(filter_order, f_bs_norm, a_bs);

% Apply Filter
filtered_noise_bs = filter(b_bs, 1, noise);

% Plot Output Spectrum
figure('Name', 'BSF Output');
plotspec(filtered_noise_bs, Ts);
title('Output Noise Spectrum After BSF (Rejecting 2-5 kHz)');
xlabel('Frequency (Hz)');
ylabel('Magnitude');